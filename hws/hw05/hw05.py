...
def is_bst(t):
    ...
    if t.is_leaf():
        return True
    children = [b for b in t.branches]
    if len(children) > 2:
        return False
    for idx, child in enumerate(children):
        if not ((child.label <= t.label and idx == 0 and bst_max(child) <= t.label) \
        or (child.label > t.label and (idx == 1 or len(children) == 1) and bst_min(child) > t.label)):
            return False
...
def bst_min(t):
    if t.is_leaf():
        return t.label
    return min([t.label] + [bst_max(b) for b in t.branches])
